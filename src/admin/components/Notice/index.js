...
const Notice = ( props ) => {
	...
	const containerClasses = classNames( className, 'dlx-pb-admin__notice', {
		'dlx-pb-admin__notice--has-icon': hasIcon(),
		[ `dlx-pb-admin__notice-type--${ status }` ]: true,
		[ `dlx-pb-admin__notice-appearance--inline` ]: inline,
		[ `dlx-pb-admin__notice-appearance--block` ]: ! inline,
	} );

	const actions = [
		{
			label: __( 'Back to Top', 'wp-dlx-pb-comments' ),
	...
	return (
		<div className={ containerClasses }>
			<WPNotice isDismissible={ false } spokenMessage={ message } actions={ hasToTop ? actions : [] } { ...props }>
				{ hasIcon() &&
					<div className="dlx-pb-admin__notice-icon">{ getIcon( icon ) }</div>
				}
				<div className="dlx-pb-admin__notice-message"><>{ message } { children } </></div>
			</WPNotice>
		</div>
	);
};
